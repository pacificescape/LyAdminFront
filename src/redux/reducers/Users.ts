...
export default (state: InitialSettingsType = initialSettings, action: any) => {
    switch (action.type) {
        ...
        case SET_USER: {
            ...
            newusers = { ...newusers, ...action.users, ...state.users[action.groupId] }
...
export const setGroupMembers = (members: object) => ({ type: SET_GROUP_MEMBERS, members })
export const setUser = (users: object, groupId: string) => ({ type: SET_USER, users, groupId })

type ToggleIsInitializedType = { type: typeof INITIALIZE_APP, isInitialized: boolean}
export const toggleIsInitialized = (isInitialized: boolean): ToggleIsInitializedType => ({ type: INITIALIZE_APP, isInitialized })

type ToggleiIsInitializingType = { type: typeof TOGGLE_IS_INITIALIZING, initializing: boolean }
...
type ToggleIsFetchingType = { type: typeof TOGGLE_IS_FETCHING, isFetching: boolean, method: string }
export const toggleIsFetching = (isFetching: boolean, method: string): ToggleIsFetchingType => ({ type: TOGGLE_IS_FETCHING, isFetching, method })
...
type ToggleisChangingSettingsType ={ type: typeof TOGGLE_IS_CHANGING_SETTINGS, isChanging: boolean }
...
export const getUserThunk = (userIds: Array<GroupMemberType>, groupId: string) => (dispatch: any) => {
